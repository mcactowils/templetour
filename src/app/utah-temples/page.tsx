...
function TempleCard({ temple, isDragging = false }: { temple: Temple; isDragging?: boolean }) {
  ...
  return (
    <div
      className={`
        bg-white rounded-xl shadow-lg border border-gray-200 p-6 transition-all duration-200
        ${isDragging ? 'opacity-50 rotate-2 scale-95' : 'hover:shadow-xl hover:-translate-y-1'}
      `}
    >
      ...
      <div className="space-y-3">
        ...
        {temple.address && (
          <div className="flex items-start text-gray-600">
            ...
            <span className="text-sm leading-relaxed">{temple.address}</span>
          </div>
        )}
        ...
        <div className="flex justify-between items-end">
          ...
          {temple.dedicationDate && (
            <div className="text-xs text-gray-500">
              Dedicated {new Date(temple.dedicationDate).getFullYear()}
            </div>
          )}
        </div>
...
export default function UtahTemplesPage() {
